                ...
                {accountTypes.map(type => (
                    (accountsByType[type] && accountsByType[type].length > 0) && (
                        <div key={type} className="bg-white shadow-md rounded-lg overflow-hidden">
                            <h2 className="px-6 py-3 text-lg font-semibold text-gray-700 bg-gray-50 border-b capitalize">{type.toLowerCase()} Accounts</h2>
                                        ...
                                        {accountsByType[type].map((account) => (
                                            <tr key={account.id}>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {account.name}
                                                    {account.isSystemAccount && <span className="ml-2 text-xs text-gray-400">(System)</span>}
                                                </td>
